...
<li>Crash and error diagnostic data, generated when the app encounters a crash or technical error, used to keep the Service stable and secure (see Section 9.7)</li>
</ul>
...
<li>Absturz- und Fehlerdiagnosedaten, die entstehen, wenn in der App ein Absturz oder technischer Fehler auftritt, und die dazu dienen, den Dienst stabil und sicher zu halten (siehe Abschnitt 9.7)</li>
</ul>
...
<p><strong>9.1 Supabase: backend hosting, authentication, database, and storage</strong><br>Data processed: account data, settings, User-generated content, AI input and output, subscription status, and technical/connection data. Purpose: hosting our application backend; authenticating users; storing the database and uploaded files (images and audio). Legal basis: Art. 6(1)(b) GDPR (and Art. 6(1)(f) for security/operation). Location: hosted on Amazon Web Services (AWS) infrastructure in the European Union (Frankfurt, Germany). Your account data, your content, and your uploaded files are therefore stored within the EU. Supabase Inc. is established in the United States, so limited remote access from outside the EEA (for example for support or maintenance) cannot be entirely excluded; such access is covered by the safeguards described in Section 14. Relationship: processor under Art. 28 GDPR.</p>
...
<p><strong>9.7 Sentry: crash and error monitoring</strong><br>Data processed: crash and error reports and related diagnostic information, such as the error type and stack trace, the app version and the screen or action in use at the time, device and operating-system information, a pseudonymous installation/user identifier, the IP address used to establish the connection, and a limited trail of preceding in-app events ("breadcrumbs") that may incidentally include content you were interacting with. Purpose: detecting, diagnosing, and fixing crashes and technical errors so we can keep the Service stable and secure. Legal basis: our legitimate interest in the stability, security, and correct operation of the Service, Art. 6(1)(f) GDPR; this is diagnostic error monitoring, not analytics or advertising, and does not require consent. Provider: Functional Software, Inc. (Sentry). Location: hosted in Sentry's European Union data region (Germany), so this does not involve a transfer to a third country. Relationship: processor under Art. 28 GDPR.</p>
<p><strong>9.8 Analytics and advertising providers (only with consent)</strong><br>See Section 11 for the detailed list and conditions.</p>
...
<p><strong>9.7 Sentry: Absturz- und Fehlerüberwachung</strong><br>Verarbeitete Daten: Absturz- und Fehlerberichte sowie zugehörige Diagnoseinformationen, etwa Fehlertyp und Stack-Trace, die App-Version und der zum jeweiligen Zeitpunkt genutzte Bildschirm bzw. die genutzte Aktion, Geräte- und Betriebssysteminformationen, eine pseudonyme Installations-/Nutzerkennung, die zum Verbindungsaufbau verwendete IP-Adresse sowie eine begrenzte Abfolge vorangegangener In-App-Ereignisse („Breadcrumbs"), die beiläufig Inhalte enthalten kann, mit denen du interagiert hast. Zweck: Erkennung, Diagnose und Behebung von Abstürzen und technischen Fehlern, um den Dienst stabil und sicher zu halten. Rechtsgrundlage: unser berechtigtes Interesse an der Stabilität, Sicherheit und dem ordnungsgemäßen Betrieb des Dienstes, Art. 6 Abs. 1 lit. f DSGVO; hierbei handelt es sich um diagnostische Fehlerüberwachung, nicht um Analyse oder Werbung, sodass keine Einwilligung erforderlich ist. Anbieter: Functional Software, Inc. (Sentry). Standort: gehostet in der EU-Datenregion von Sentry (Deutschland), sodass keine Übermittlung in ein Drittland erfolgt. Verhältnis: Auftragsverarbeiter nach Art. 28 DSGVO.</p>
<p><strong>9.8 Analyse- und Werbeanbieter (nur mit Einwilligung)</strong><br>Die detaillierte Liste und die Bedingungen findest du in Abschnitt 11.</p>
...
<p>Hinweis: Einige dieser Tools sind geplant und möglicherweise nicht immer aktiv. Wir halten diese Erklärung aktuell und verarbeiten Daten über ein bestimmtes Tool erst, sobald es aktiv ist und du eingewilligt hast. Du kannst deine Einwilligung jederzeit über die Datenschutz-/Einwilligungseinstellungen in der App verwalten oder widerrufen (siehe Abschnitt 18).</p>`,
      ...
      },
      {
        en: {
          ...
          body: `<p>Our database, our authentication service, and the files you upload are hosted on AWS infrastructure in the European Union (Frankfurt, Germany), and our crash and error monitoring runs in Sentry's European Union data region (Germany). This content is therefore stored within the EU and is not routinely transferred to a third country.</p>
<p>Some of our other service providers are located in, or process data in, the United States or other countries outside the EEA, which may not provide a level of data protection equivalent to that of the EU. In particular, our AI, subscription, and security/anti-abuse (Cloudflare Turnstile) providers, as well as our (consent-based) analytics and advertising providers, may process data in the United States. In addition, some of the providers that host data for us in the EU belong to groups established in the United States, so limited remote access from outside the EEA, for example for support or maintenance, cannot be entirely excluded.</p>
...
<p>You can request a copy of the relevant safeguards or further information about these transfers by contacting us at neomind.help@gmail.com.</p>`,
        ...
        de: {
          ...
          body: `<p>Unsere Datenbank, unser Authentifizierungsdienst und die von dir hochgeladenen Dateien werden auf AWS-Infrastruktur in der Europäischen Union (Frankfurt, Deutschland) gehostet, und unsere Absturz- und Fehlerüberwachung läuft in der EU-Datenregion von Sentry (Deutschland). Diese Inhalte werden somit innerhalb der EU gespeichert und nicht regelmäßig in ein Drittland übermittelt.</p>
<p>Einige unserer übrigen Dienstleister befinden sich in den Vereinigten Staaten oder anderen Ländern außerhalb des EWR oder verarbeiten dort Daten, die möglicherweise kein der EU gleichwertiges Datenschutzniveau bieten. Insbesondere können unsere Anbieter für KI, Abonnements und Sicherheit/Missbrauchsschutz (Cloudflare Turnstile) sowie unsere (einwilligungsbasierten) Analyse- und Werbeanbieter Daten in den Vereinigten Staaten verarbeiten. Zudem gehören einige der Anbieter, die Daten für uns in der EU hosten, zu Unternehmensgruppen mit Sitz in den Vereinigten Staaten, sodass ein eingeschränkter Fernzugriff von außerhalb des EWR, etwa zu Support- oder Wartungszwecken, nicht vollständig ausgeschlossen werden kann.</p>
